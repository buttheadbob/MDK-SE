...
|Member|Description|
|---|---|
|[Radius { get; set; }](Sandbox.ModAPI.Ingame.IMyBeacon.Radius)|Broadcasting range (read-only)|
|[HudText { get; set; }](Sandbox.ModAPI.Ingame.IMyBeacon.HudText)|Gets or sets the text to display on the HUD when the beacon is on.|
