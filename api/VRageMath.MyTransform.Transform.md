← [Index](Api-Index) ← [Namespace Index](Namespace-Index) ← [MyTransform](VRageMath.MyTransform)

### Summary

```csharp
public static MyTransform Transform(ref MyTransform t1, ref MyTransform t2)
```

### Returns

[MyTransform](VRageMath.MyTransform)

### Parameters

* [MyTransform](VRageMath.MyTransform) t1
* [MyTransform](VRageMath.MyTransform) t2
### Summary

```csharp
public static void Transform(ref MyTransform t1, ref MyTransform t2, ref MyTransform result)
```

### Parameters

* [MyTransform](VRageMath.MyTransform) t1
* [MyTransform](VRageMath.MyTransform) t2
* [MyTransform](VRageMath.MyTransform) result
### Summary

```csharp
public static Vector3 Transform(ref Vector3 v, ref MyTransform t2)
```

### Returns

[Vector3](VRageMath.Vector3)

### Parameters

* [Vector3](VRageMath.Vector3) v
* [MyTransform](VRageMath.MyTransform) t2
### Summary

```csharp
public static void Transform(ref Vector3 v, ref MyTransform t2, ref Vector3 result)
```

### Parameters

* [Vector3](VRageMath.Vector3) v
* [MyTransform](VRageMath.MyTransform) t2
* [Vector3](VRageMath.Vector3) result
