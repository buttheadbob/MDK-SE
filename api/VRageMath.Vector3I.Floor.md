← [Index](Api-Index) ← [Namespace Index](Namespace-Index) ← [Vector3I](VRageMath.Vector3I)

### Summary

```csharp
public Vector3I Floor(Vector3 value)
```

### Returns

[Vector3I](VRageMath.Vector3I)

### Parameters

* [Vector3](VRageMath.Vector3) value
### Summary

```csharp
public Vector3I Floor(Vector3D value)
```

### Returns

[Vector3I](VRageMath.Vector3I)

### Parameters

* [Vector3D](VRageMath.Vector3D) value
### Summary

```csharp
public void Floor(ref Vector3 v, ref Vector3I r)
```

### Parameters

* [Vector3](VRageMath.Vector3) v
* [Vector3I](VRageMath.Vector3I) r
### Summary

```csharp
public void Floor(ref Vector3D v, ref Vector3I r)
```

### Parameters

* [Vector3D](VRageMath.Vector3D) v
* [Vector3I](VRageMath.Vector3I) r
