← [Index](Api-Index) ← [Namespace Index](Namespace-Index) ← [Vector2D](VRageMath.Vector2D)

### Summary

```csharp
public static Vector2D Divide(Vector2D value1, Vector2D value2)
```

Divides the components of a vector by the components of another vector.

### Returns

[Vector2D](VRageMath.Vector2D)

### Parameters

* [Vector2D](VRageMath.Vector2D) value1
* [Vector2D](VRageMath.Vector2D) value2
### Summary

```csharp
public static void Divide(ref Vector2D value1, ref Vector2D value2, ref Vector2D result)
```

Divides the components of a vector by the components of another vector.

### Parameters

* [Vector2D](VRageMath.Vector2D) value1
* [Vector2D](VRageMath.Vector2D) value2
* [Vector2D](VRageMath.Vector2D) result
### Summary

```csharp
public static Vector2D Divide(Vector2D value1, double divider)
```

Divides a vector by a scalar value.

### Returns

[Vector2D](VRageMath.Vector2D)

### Parameters

* [Vector2D](VRageMath.Vector2D) value1
* [double](https://docs.microsoft.com/en-us/dotnet/api/system.double?view=netframework-4.6) divider
### Summary

```csharp
public static void Divide(ref Vector2D value1, double divider, ref Vector2D result)
```

Divides a vector by a scalar value.

### Parameters

* [Vector2D](VRageMath.Vector2D) value1
* [double](https://docs.microsoft.com/en-us/dotnet/api/system.double?view=netframework-4.6) divider
* [Vector2D](VRageMath.Vector2D) result
