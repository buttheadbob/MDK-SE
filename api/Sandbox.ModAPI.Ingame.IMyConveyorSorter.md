...
|Member|Description|
|---|---|
|[DrainAll](Sandbox.ModAPI.Ingame.IMyConveyorSorter.DrainAll)|Determines whether the sorter should drain any inventories connected to it and push them to the other side - as long as the items passes the filtering as defined by the filter list ( [GetFilterList(List<Sandbox.ModAPI.Ingame.MyInventoryItemFilter>)](Sandbox.ModAPI.Ingame.IMyConveyorSorter.GetFilterList) ) and [Mode](Sandbox.ModAPI.Ingame.IMyConveyorSorter.Mode) .|
