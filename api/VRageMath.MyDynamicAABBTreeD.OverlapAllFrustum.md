← [Index](Api-Index) ← [Namespace Index](Namespace-Index) ← [MyDynamicAABBTreeD](VRageMath.MyDynamicAABBTreeD)

### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, List<T> elementsList, bool clear)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [List<T>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) elementsList
* [bool](https://docs.microsoft.com/en-us/dotnet/api/system.boolean?view=netframework-4.6) clear
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, List<T> elementsList, uint requiredFlags, bool clear)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [List<T>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) elementsList
* [uint](https://docs.microsoft.com/en-us/dotnet/api/system.uint32?view=netframework-4.6) requiredFlags
* [bool](https://docs.microsoft.com/en-us/dotnet/api/system.boolean?view=netframework-4.6) clear
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, List<T> elementsList, List<System.Boolean> isInsideList)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [List<T>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) elementsList
* [List<System.Boolean>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) isInsideList
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, T results)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [T]() results
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, List<T> elementsList, List<System.Boolean> isInsideList, float tSqr, bool clear)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [List<T>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) elementsList
* [List<System.Boolean>](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list?view=netframework-4.6) isInsideList
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) tSqr
* [bool](https://docs.microsoft.com/en-us/dotnet/api/system.boolean?view=netframework-4.6) clear
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, Action<T, System.Boolean> add, float tSqr)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [Action<T, System.Boolean>](https://docs.microsoft.com/en-us/dotnet/api/system.action?view=netframework-4.6) add
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) tSqr
### Summary

```csharp
public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, T results, float tSqr)
```

### Parameters

* [BoundingFrustumD](VRageMath.BoundingFrustumD) frustum
* [T]() results
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) tSqr
