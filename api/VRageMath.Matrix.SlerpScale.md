← [Index](Api-Index) ← [Namespace Index](Namespace-Index) ← [Matrix](VRageMath.Matrix)

### Summary

```csharp
public static void SlerpScale(ref Matrix matrix1, ref Matrix matrix2, float amount, ref Matrix result)
```

Performs spherical linear interpolation of position and rotation and scale.

### Parameters

* [Matrix](VRageMath.Matrix) matrix1
* [Matrix](VRageMath.Matrix) matrix2
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) amount
* [Matrix](VRageMath.Matrix) result
### Summary

```csharp
public static void SlerpScale(Matrix matrix1, Matrix matrix2, float amount, ref Matrix result)
```

Performs spherical linear interpolation of position and rotation and scale.

### Parameters

* [Matrix](VRageMath.Matrix) matrix1
* [Matrix](VRageMath.Matrix) matrix2
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) amount
* [Matrix](VRageMath.Matrix) result
### Summary

```csharp
public static Matrix SlerpScale(Matrix matrix1, Matrix matrix2, float amount)
```

Performs spherical linear interpolation of position and rotation and scale.

### Returns

[Matrix](VRageMath.Matrix)

### Parameters

* [Matrix](VRageMath.Matrix) matrix1
* [Matrix](VRageMath.Matrix) matrix2
* [float](https://docs.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.6) amount
